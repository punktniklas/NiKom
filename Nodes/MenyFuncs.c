...
extern char menyarg[];
extern char menyinmat[];
extern char filnamn[];
extern struct Meny *activemenu, *backupmenu;
extern struct Meny *menylista, *numeny;
extern struct UserCfg *ucfg;
extern char tempbuffer[], temp[];
extern struct keywordlist *menarg;
...
/* MENYNOD */
       struct menykommando *menykmdpek;
       char bokstav, *action;
       int i=0, j, status=0, menystatus;
       int funknum;
       BOOL cmailstatus;
       long tid;
/* END MENYNOD */
...
/* MENYNOD */
        ...
        {
            if(strlen(menyinmat)<1 || ucfg->hotkeys==TRUE)
            {
                            if(ucfg->expert==TRUE)
                            printmenutext(activemenu,EXPERTMODE);
                    else
                            printmenutext(activemenu,NOEXPERTMODE);
            }
        }

        cmailstatus=checkmail();
        if(cmailstatus || kmd!=306)
        {
                Servermem->action[nodnr] = LASER;
                Servermem->varmote[nodnr] = mote2;
        }
        if(mote2==-1)
        {
                if(varmailstatus==FALSE)
                        varmail();
                varmailstatus=FALSE;
        }
        if(cmailstatus && activemenu->ret!=READBREV)
        {
                puttekn(go_mailbox,-1);
        }
        else if(kmd!=306 && activemenu->ret!=READTEXT)
        {
                puttekn(unread_texts,-1);
        }
        else
                Servermem->action[nodnr] = INGET;

        puttekn("\r\n",-1);
        if(activemenu->visamote==TRUE)
        {
                if(mote2==-1)
                {
...
/* MENYNOD */
        ...
        if(readonlymode == FALSE)
        {
            if(strlen(menyinmat)<1 && ucfg->hotkeys==FALSE)
            {
/* END MENYNOD */
               if(getstring(EKO,99,NULL)) return(-8);
/* MENYNOD */
			   printf("inmat = %s\n",inmat);
               strcpy(menyinmat,inmat);
               i=0;
            }
            else
            {
        ...
        if(readonlymode==FALSE)
        {
            if(menyinmat[0]==menylista->showmenukey)
            {
                printmenutext(activemenu,NOEXPERTMODE);
                goto menyinloop;
            }
        ...
        else
        {
            ...
            else if(menyinmat[0]=='Q' || menyinmat[0]=='q')
            {
                readonlymode=FALSE;
                if(ucfg->scrclr==TRUE)
                    puttekn("\x1b[2J \x1b[0;0H",-1);
        ...
        while(i<strlen(menyinmat))
        {
                if( (activemenu->ret==READBREV || activemenu->ret==READTEXT)
                   && ucfg->hotkeys==TRUE
                   && menyinmat[0]=='\r')
                {
                        if(ucfg->scrclr==TRUE)
                ...
                status=1;
                bokstav=menyinmat[i];
                for(j=0;j<strlen(menyinmat);j++)
                        menyinmat[j]=menyinmat[j+1];
                action=checkchoise(activemenu->key, bokstav);
                checkarg(activemenu->key, bokstav);
                if(action==NULL)
                {
                        sprintf(outbuffer,"%s\n",illegal_choise);
                        puttekn(outbuffer,-1);
                ...
                else
                {
                        ...
                        if(funknum==0)
                        {
                                backupmenu=activemenu;
                                activemenu=changemenu(menylista,action, TRUE);
                                if(activemenu==NULL)
                                        activemenu=backupmenu;
                                else
                                {
                                    if(activemenu->startkom != NULL)
                                    {
                                        funknum = atoi(activemenu->startkom->name);
                                        menarg=activemenu->startkom;
                                    }
                                }
                        }
                        if(funknum != 0 )
                        {
                                menyarg[0]=0;
                                if(menarg->arg=='Y' || menarg->arg =='y')
                                        if(setmenyarg(funknum,menarg->visastring)) return(-8);
                                if(menarg->argument!=NULL)
                                {
                                        strcpy(tempbuffer,menarg->argument);
                                        if(menyarg[0]!=0);
                                                strcat(tempbuffer,menyarg);
                                        strcpy(menyarg,tempbuffer);
                                }
                                argument=menyarg;
                                ...
                                menykmdpek=NULL;
                                menystatus=0;
                                if( (funknum<100 && funknum>9) || menygetkmdpek(funknum) != NULL)
                                {
                                        menykmdpek = menygetkmdpek(funknum);
                                        if(menykmdpek==NULL)
                                            return(-5);
                                        if(menykmdpek->before)
                                            sendautorexx(menykmdpek->before);
                                        ...
                                        if(menykmdpek->vilkainfo[0])
                                        {
                                            ...
                                            Servermem->vilkastr[nodnr] = menykmdpek->vilkainfo;
                                        }
                                        menystatus=1;
                                }
                                else
                                {
                                        kmdpek = getkmdpek(funknum);
                                        if(!kmdpek) return(-5);
                                ...
                                kmdret=menykmd(funknum, kmd,menystatus);
                                if(menykmdpek!=NULL) if(menykmdpek->after) sendautorexx(menykmdpek->after);
                                else if(funknum>99 && kmdpek->after) sendautorexx(kmdpek->after);
                                return(kmdret);
                        }
                }
        }
        if(status==0 && (activemenu->ret==READBREV || activemenu->ret==READTEXT) )
        {
                if(ucfg->scrclr==TRUE)
                        puttekn("\x1b[2J \x1b[0;0H",-1);
                return(-6);
...
/* MENYNOD
        ...
        if(inmat[0] && (aliaspek=parsealias(inmat))) {
                ...
                strcat(aliasbuf," ");
                strncat(aliasbuf,hittaefter(inmat),158);
...
int dokmd(int parseret,int kmd) {
        ...
        else if(parseret==101) listmot(NULL);
...
*/
        else if(parseret==323) aboutmeny();
...
int ga(char *foo) {
        ...
        if(motnr==-1) {
                ...
                puttekn("\r\n\nDu är inte medlem i det mötet!\r\nSkriv: Medlem <mötesnamn> för att bli det.\r\n",-1);
                return(-5);
        }
        return(motnr);
}
...
/* MENYNOD */
        ...
        if(!(ucfg=malloc(sizeof(struct UserCfg))))
        {
                sprintf(outbuffer,"%s\n\n",severe_error);
                puttekn(outbuffer,-1);
        }
        ucfg->name=NULL;
        status=readuserconfig(inloggad);
        if(status==FALSE)
        {
                sprintf(outbuffer,"%s\n\n",severe_error);
                puttekn(outbuffer,-1);
...
/* END MENYNOD */
        ...
        for(;;) {
                ...
                else if(promret==-6 && activemenu->ret==READTEXT) tiden();
                /* MENYNOD */
                else if(promret==-6 && activemenu->ret==READBREV)
                {
                        mote2=-1;
                        var(mote2);
                }
